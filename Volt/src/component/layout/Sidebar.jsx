      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
        <div>
          ...
          <div className="space-y-1">
            <NavItem icon={<Users size={18} />} label="Clients" active />
            <NavItem icon={<FileText size={18} />} label="Proposals" />
        ...
        <div>
          ...
          <div className="space-y-1">
            <NavItem icon={<Landmark size={18} />} label="Bank updates" />
            <NavItem icon={<Briefcase size={18} />} label="Bank products" />
            <NavItem icon={<FileStack size={18} />} label="Documents center" />
            <NavItem icon={<Users size={18} />} label="Refer and Earn" />
            <NavItem icon={<PlayCircle size={18} />} label="Tutorials" />
          </div>
        </div>
      </div>
...
const NavItem = ({ icon, label, active = false }) => (
  <div className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${active ? 'bg-white shadow-sm border border-gray-100 text-slate-900' : 'text-gray-500 hover:bg-gray-50 hover:text-slate-900'}`}>
    <span className={active ? 'text-slate-900' : 'text-gray-400'}>{icon}</span>
